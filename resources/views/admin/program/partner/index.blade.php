<x-app-layout>
    <div>
        <div class="flex justify-between">
            <div class="">
                <h1 class="text-2xl font-semibold">Managemen Konten Kerja Sama</h1>
                <p class="text-sm font-medium">Pilih Konten untuk mengedit :</p>
            </div>
            <a href="{{ route('admin.program.index') }}" class="hidden font-medium md:block">
                <svg
            ...
            <div class="mt-5 grid grid-cols-3 gap-3 md:grid-cols-6 lg:grid-cols-9">
                ...
                <div class="col-span-3 overflow-hidden rounded-3xl border-2 p-4">
                    <div
                        ...
                        <img
                            class="mx-auto h-full w-2/4 object-cover"
                            src="{{ $partner->photo ? asset('storage/' . $partner->photo) : '' }}"
                            alt="Partner Image" />
                    </div>
                    <div class="text-base">
                        <p class="line-clamp-1 font-bold text-gray-900">{{ $partner->name }}</p>
                        <p class="line-clamp-1 text-gray-500">{{ $partner->program }}</p>
                    </div>
                    <div class="mt-5 flex justify-evenly space-x-1 md:px-4">
                        <form action="{{ route('admin.partner.destroy', $partner) }}" method="POST" class="w-full">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                        ...
                        <a
                            href="{{ route('admin.partner.edit', $partner) }}"
