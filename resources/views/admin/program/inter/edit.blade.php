<x-app-layout>
    <div>
        <div class="flex justify-between">
            <div class="">
                <h1 class="text-2xl font-semibold">Edit MBKM dan Magang</h1>
                <p class="text-sm font-medium">Isi Konten untuk edit program :</p>
            </div>
            <a
        ...
        <form action="{{ route('admin.intern.update', $intern) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="basis-full md:basis-7/12">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea
                            ...
                            rows="4"
                            class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500"
                            placeholder="">{{ old('description', $intern->description) }}</textarea>
                        @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="mb-6">
                        ...
                        <div id="projects-container">
                            @forelse($intern->projects as $key => $project)
                            <div class="mb-3 flex items-center gap-2">
                                <input
                                    type="text"
                                    name="projects[]"
                                    value="{{ old('projects.'.$key, $project->name) }}"
                                    class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500" />
                                <button
                                    type="button"
                    ...
                    <div class="mb-6">
                        ...
                        <div id="majors-container">
                            @forelse($intern->majors as $key => $major)
                            <div class="mb-3 flex items-center gap-2">
                                <input
                                    type="text"
                                    name="majors[]"
                                    value="{{ old('majors.'.$key, $major->name) }}"
                                    class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500" />
                                <button
                                    type="button"
                                    class="remove-field rounded-lg bg-red-500 px-3 py-2 text-white">
                ...
                <div class="basis-full md:basis-5/12">
                    <div class="mb-6">
                        <label for="photo" class="mb-2 block text-sm font-medium text-gray-900">
                            Poster Program Saat Ini
                        </label>
                        <div class="mb-4 h-64 overflow-hidden rounded-lg">
                            <img src="{{ $intern->photo_url }}" alt="{{ $intern->name }}" class="h-full w-full object-cover" />
                        </div>

                        <label for="photo" class="mb-2 block text-sm font-medium text-gray-900">
                            Ubah Poster Program (Opsional)
                        </label>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Add project field
            document.querySelectorAll('.add-project').forEach(function(button) {
                button.addEventListener('click', function() {
                    ...
                    // Insert the new row before the "+" button container
                    const addButtonContainer = container.querySelector('.add-project').parentElement;
            ...
            // Add major field
            document.querySelectorAll('.add-major').forEach(function(button) {
                button.addEventListener('click', function() {
                    ...
                    // Insert the new row before the "+" button container
                    const addButtonContainer = container.querySelector('.add-major').parentElement;
                    container.insertBefore(newRow, addButtonContainer);
